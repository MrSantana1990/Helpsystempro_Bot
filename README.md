# HelpSystem • Binance Bot (padrão HelpSystem)

## Rodar (Windows / PowerShell)
Comando único (garantido) para ver o **portal** (mesmo padrão visual do `D:\DEV\Helpsystem_Pro`) com dados fictícios:
- `cd D:\DEV\Helpsystempro_Bot; powershell -NoProfile -ExecutionPolicy Bypass -File .\run_local.ps1`

Obs (PowerShell): para rodar scripts no diretório atual, use `.\` (ex.: `.\run_local.ps1`). Se seu ExecutionPolicy bloquear, use o comando acima com `-ExecutionPolicy Bypass`.

Sem mock (padrão): o painel sobe sem dados fictícios. Para gerar dados demo:
- `.\run_local.ps1 -Mock -Seed 42`

1) Copie `BinanceBot/Configs/key.env.example` para `BinanceBot/Configs/key.env` e preencha as chaves.
2) Instale dependências (venv):
   - `py -m venv .venv`
   - `.\.venv\Scripts\python -m pip install -r requirements.txt`
3) Rodar bot (recomendado começar em simulação):
   - `.\run_bot.ps1`
   - ou `.\.venv\Scripts\python BinanceBot\Binance_Bot.py --dry-run`
   - Para operar em conta real (testnet=false): defina `HSP_LIVE_TRADING=1` (trava de segurança).
4) Rodar painel:
   - `.\run_panel.ps1`
   - ou `.\.venv\Scripts\python -m streamlit run BinanceBot\dashboard.py --server.port 8503`

## Portal (visual HelpSystem Pro)
Painel moderno em **React** (visual tipo Binance):
- `cd D:\DEV\Helpsystempro_Bot; powershell -NoProfile -ExecutionPolicy Bypass -File .\run_local.ps1`
  - O script define `HSP_PORTAL_TOKEN=local-dev` (use esse token para salvar config e start/stop).
- Abrir painel: `http://localhost:8501`
- API (backend): `http://localhost:8502`
- Streamlit antigo (opcional): `http://localhost:8503`

Smoke test da API:
- `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\smoke_test.ps1`

## Como funciona (resumo)
- O bot calcula sinais (RSI/Bollinger/MACD + sentimento de notícias), gera um **score** e grava a decisão (com explicação).
- Trades e decisões ficam no SQLite: `data/trading.sqlite3`.
- O painel mostra: Visão Geral, Trades, Decisões (explicação), Config, Logs e Ferramentas.
